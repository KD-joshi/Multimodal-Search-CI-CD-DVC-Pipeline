...
class TextEmbedder:
    ...
    def _build_text(self, df: pd.DataFrame) -> List[str]:
        ...
        Concatenates: product_name | brand | color | top categories | meta_keywords.
        Categories are truncated to the first 3 tokens and meta_keywords to 150 chars
        to stay within the model's 512-token context window.
        """
        texts = []
        for _, row in df.iterrows():
            ...
            # Categories: take first 3 words (enough to know the type)
            cats_raw = str(row.get('categories', ''))
            cats = ' '.join(cats_raw.split()[:3])
            
            # Meta keywords: often product name + brand + keywords, cap at 150 chars
            keywords = str(row.get('meta_keywords', ''))[:150]
            
            parts = [p for p in [name, brand, color, cats, keywords] if p.strip()]
            text = " | ".join(parts)
            texts.append(text)
        return texts
